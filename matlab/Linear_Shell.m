function average_Hz = Linear_Shell(Field_Str,Noise_Str)
    N = 64;
    mid = round(N/2);
    L = 70; % <-- [pc]
    ...
    % Magnetic field
    Bx0 = Field_Str * ones(N,N,N) + Noise_Str * randn(N,N,N) / 3^0.5;
    By0 = zeros(N,N,N) + Noise_Str * randn(N,N,N) / 3^0.5;
    Bz0 = zeros(N,N,N) + Noise_Str * randn(N,N,N) / 3^0.5;
